...
test_assert_db_normal(){
	...
	assert_db_with_conv dd EMP "${SSU_SELFPATH}" "where EID='EID'"
	assert_db_with_conv dd EMP "${SSU_SELFPATH}" "where EID='EID'" pooh
	...
	w="where EID='0'"
	assert_db_with_conv dd EMP "${SSU_SELFPATH}" "$w" pooh > ee 2> /dev/null &
	typeset jid=$!;
	wait $jid;
	typeset r=$?
	...
	#assert_exit_code "assert_db_with_conv uuu EMP "${SSU_SELFPATH}" "where EID='0'" pooh" 99 > ee 2> /dev/null
	w="where EID='0'"
	assert_db_with_conv uuu EMP "${SSU_SELFPATH}" "$w" pooh > ee 2> /dev/null &
	jid=$!;
	wait $jid;
	r=$?
	...
	h_mkdir iii
	#assert_exit_code "assert_db iii EMP "${SSU_SELFPATH}"  'where EID='0'' pooh" 99 > ee 2> /dev/null
	w="where EID='0'"
	assert_db_with_conv iii EMP "${SSU_SELFPATH}" "$w" pooh > ee 2> /dev/null &
	jid=$!;
	wait $jid;
	r=$?
	...
	#assert_exit_code "assert_db bb EMP99999 "${SSU_SELFPATH}" "where EID='0'" pooh" 99 > ee 2> /dev/null
	w="where EID='0'"
	assert_db_with_conv bb EMP99999 "${SSU_SELFPATH}" "$w" pooh > ee 2> /dev/null &
	jid=$!;
	wait $jid;
	r=$?
	...
	#assert_exit_code "assert_db bb EMP99999 "${SSU_SELFPATH}" EID='0' pooh 3 2 3" 99 > ee 2> /dev/null
...
date
#TARGET_TEST_PATTERN="test_assert_db_normal_FROMfile"
#DEBUG_MODE=ON
startSSU;
date
